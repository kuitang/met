...
`JOURNEY_TARGET=https://musewalk.app` runs the same suite against the deployed app. `@live` journey variants (real Gemini instead of LLM_MOCK) run only with `LLM_LIVE=1` against a non-mock server. After a journeys run, `node collect-videos.mjs` copies each video to `recordings/J{n}-{slug}.webm`. Helpers: `helpers/geo.ts` (simulated walking/GPS), `helpers/steps.ts` (captioned steps), `helpers/journey.ts` (real-stack boot + room taps), `helpers/db.ts` (fixtures read live from `data/met.sqlite`, so journey assertions survive the partial→full hydration); photo fixtures live in `fixtures/`. The app contract: root view exposes `testID="app-root"`.
