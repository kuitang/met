...
- **The nightly job** (`data/src/nightly.ts`, GHA `nightly-data.yml`, also
  ...
  deltas** — never an in-job `fullFetch` (measured 2026-07-06: the old
  first-night fallback burned 3.8 h/night re-crawling the bot-walled Louvre,
  and any slow source risked timing out the whole 6 h job, shipping
  nothing). Each museum's stage snapshots layer: the git-committed
  `data/museums/{id}/snapshots/` dir first (the onboarding harvest + side
  files like the `translations.json` cache), then last night's artifact rows
  on top. Onboarding a new museum is therefore local-first: run its adapter
  harvest on a dev machine and **commit the snapshot** — the next nightly
  deltas from the committed harvest's `fetchedAt` and ships it; a museum
  with neither artifact rows nor a committed snapshot ships empty with a
  loud log.
